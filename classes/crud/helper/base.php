...
use core\persistent;
use local_cltools\crud\persistent_navigation;
...
abstract class base {
    ...
    public function instanciate_related_persistent_list() {
        ...
        $actionsdefs = [
            'edit' => (object) [
                'icon' => 't/edit',
                'url' => persistent_navigation::get_edit_url($this->refpersistentclass)
            ],
            'view' => (object) [
                'icon' => 'e/search',
                'url' => persistent_navigation::get_view_url($this->refpersistentclass),
            ],
            'delete' => (object) [
                'icon' => 't/delete',
                'url' => persistent_navigation::get_delete_url($this->refpersistentclass)
            ]
        ];
    ...
    protected function setup_page_navigation($page) {
        ...
        $header = $this->get_page_header();;
        $listpageurl = new moodle_url($this->get_list_url());
        $page->navbar->add(
            $entities,
    ...
    protected function get_page_header() {
        $stringprefix = persistent_utils::get_persistent_prefix($this->refpersistentclass);
        $action = get_string($this->action, 'local_cltools');
