...
use local_cltools\form\persistent_list_filter;
use moodle_url;
use pix_icon;
use popup_action;
use table_sql;
...
abstract class persistent_list extends table_sql {
    ...
    protected static function add_all_definition_from_persistent(&$existingproperties) {
        ...
        foreach (static::$persistentclass::properties_definition() as $name => $prop) {
            ...
            $label = '';
            if ($stringmanager->string_exists($entityprefix . ':' . $name, 'local_cltools')) {
                $label = get_string($entityprefix . ':' . $name, 'local_cltools');
            } else {
                $label = get_string($name, 'local_cltools');
            }
