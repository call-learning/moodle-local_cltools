...
namespace local_cltools\crud\form;
...
abstract class persistent_form extends \core\form\persistent {
    ...
    protected function filter_data_for_persistent($data) {
        $fieldstoremove = array_flip((array) static::$foreignfields);
        $allfilefields = $this->get_file_fields();
        foreach ($allfilefields as $field) {
            foreach ($this->_form->_elements as $e) {
                if ($e->getType() == 'editor') {
                    $fieldstoremove[$e->getName()] = $e->getName();
                }
            }
        }
        return (object) array_diff_key((array) $data, $fieldstoremove);
    }
    ...
    protected function get_mix_persistent_form_properties() {
        ...
        foreach ($allproperties as $name) {
            if (empty($properties[$name])) {
                switch ($formproperties[$name]->type) {
                    case 'file_manager':
            ...
            $forminfo = null;
            if (empty($formproperties[$name])) {
                $forminfo = new \stdClass();
                $forminfo->type = 'hidden';
                $forminfo->rawtype = $prop['type'];
            } else {
                $forminfo = $formproperties[$name];
            }

            // Uniformise the name.
            if (empty($forminfo->fullname)) {
                $forminfo->fullname = get_string($name, 'local_cltools');
            }
            $possibledefault = '';
